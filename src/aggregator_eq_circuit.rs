...
use pasta_curves::pallas::Affine as EqAffine; // aqui trocamos para EqAffine
use pasta_curves::pallas::Scalar as Fr;

/// Configuração do AggregatorCircuit com EqAffine
#[derive(Clone)]
pub struct AggregatorEqConfig {
    proof_valid: Column<Advice>,
    all_valid: Column<Advice>,
}

/// AggregatorCircuit recursivo que verifica provas externas usando EqAffine
#[derive(Clone)]
pub struct AggregatorEqCircuit {
    pub sub_proofs: Vec<Vec<u8>>,            // bytes de provas externas
    pub sub_public_inputs: Vec<Vec<Fr>>,     // inputs públicos de cada prova
    pub sub_vks: Vec<VerifyingKey<EqAffine>>, // VK de cada prova
    pub params: Params<EqAffine>,
}

/// Gadget que verifica uma prova externa e retorna AssignedCell<Fr, Fr>
fn verify_proof_gadget_eq(
    layouter: &mut impl Layouter<Fr>,
    config: &AggregatorEqConfig,
    proof_bytes: &[u8],
    public_inputs: &[Fr],
    ...
    let mut reader = Blake2bRead::<_, _, Challenge255<EqAffine>>::init(&mut proof_slice);
...
impl Circuit<Fr> for AggregatorEqCircuit {
    type Config = AggregatorEqConfig;
    ...
    fn configure(meta: &mut ConstraintSystem<Fr>) -> Self::Config {
        ...
        AggregatorEqConfig { proof_valid, all_valid }
    }
    ...
    fn synthesize(
        ...
        for (i, proof_bytes) in self.sub_proofs.iter().enumerate() {
            ...
            let verified_cell = verify_proof_gadget_eq(
                &mut layouter,
                &config,
                proof_bytes,
                public_inputs,
        ...
        println!("✅ Todas as provas agregadas e validadas com sucesso usando EqAffine!");
        Ok(())
    }
}
